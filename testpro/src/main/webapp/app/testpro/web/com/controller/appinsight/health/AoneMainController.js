Ext.define('Testpro.testpro.web.com.controller.appinsight.health.AoneMainController', {
     extend: 'Testpro.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.AoneMainController',
     init: function() {
          var form = this.view.down('#AoneForm');
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#AoneTree');
               ...
               var compRef = this.view.down('#AoneTree');
     ...
     resetForm: function(but) {
          ...
          form.down('#saveFormButton').setText('Save');
          form.reset();
          var grid = this.view.down('#AoneGrid');
          var tree = this.view.down('#AoneTree');
          tree.setSelection();
          grid.setSelection();
     },
     hideDataBrowser: function() {
          var form = this.view.down('#Aone');
          var grid = this.view.down('#AoneGrid');
          var tree = this.view.down('#AoneTreeContainer');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#AoneForm');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, opts) {
                    ...
                    var compRef = currentView.down('#AoneTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          ...
          if (record.data.leaf) {
               var Aone = this.view.down('#AoneForm');
               Aone.reset();
               var gridPanel = this.view.down('#AoneGrid');
          ...
          var formPanel = this.getView().up().down('#AoneForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#AoneTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Aone').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#AoneGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
