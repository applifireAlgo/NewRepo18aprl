Ext.define('Testpro.testpro.web.com.controller.appinsight.health.AtwoMainController', {
     extend: 'Testpro.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.AtwoMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#AtwoTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#AtwoForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#AtwoGrid');
               var tree = this.view.down('#AtwoTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#AtwoForm');
          var grid = this.view.down('#AtwoGrid');
          var tree = this.view.down('#AtwoTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#AtwoGrid');
               ...
               var formPanel = this.getView().up().down('#AtwoForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#AtwoTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#AtwoForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#AtwoGrid').store;
          gridStore.removeAll();
