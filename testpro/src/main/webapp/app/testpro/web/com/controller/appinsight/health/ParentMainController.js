Ext.define('Testpro.testpro.web.com.controller.appinsight.health.ParentMainController', {
     ...
     resetForm: function(but) {
          ...
          tree.setSelection();
          grid.setSelection();
          var gridChild = but.up('form').down('#ChildGrid');
          gridChild.getStore().removeAll();
          gridChild.reconfigure();
     },
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, opts) {
                    ...
                    var compRef = currentView.down('#ParentTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          ...
          if (record.data.leaf) {
               var Parent = this.view.down('#ParentForm');
               Parent.reset();
               var gridPanel = this.view.down('#ParentGrid');
          ...
          var ChildGrid = formPanel.down('#ChildGrid');
          ChildGrid.store.loadData(data.child);
          ChildGrid.setSelection(0);
     ...
     addChild: function(but) {
          var formChild = but.up().down('form');
          var gridChild = but.up().up().down('#ChildGrid');
          if (formChild.isValid()) {
               var values = this.getData(formChild);
               values = values[gridChild.bindable];
               if (gridChild.selection) {
                    gridChild.selection.data = values;
               } else {
                    gridChild.getStore().add(values);
               }
               gridChild.reconfigure();
               formChild.reset();
               gridChild.setSelection();
          }
     },
     onChildGridItemClick: function(cellModel, record, rowIndex, columnIndex, eOpts, comp) {
          var form;
          if (comp != null) {
               form = comp.up();
          } else {
               form = this.view.down('#ChildForm');
          }
     ...
     saveForm: function(but, evt) {
          ...
          var ChildForm = form.down('#ChildForm');
          var Child = form.down('#ChildGrid');
          if (!(ChildForm.isValid() || Child.store.getCount() != 0)) {
               this.showCard(form, ChildForm);
               return;
          }
          this.addChild(ChildForm.down('button'));
