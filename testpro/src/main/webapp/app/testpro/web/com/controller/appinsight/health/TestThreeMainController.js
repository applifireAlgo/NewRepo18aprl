Ext.define('Testpro.testpro.web.com.controller.appinsight.health.TestThreeMainController', {
     extend: 'Testpro.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.TestThreeMainController',
     init: function() {
          var form = this.view.down('#TestThreeForm');
     ...
     genLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               success: function(response, scope) {
                    var compRef = scope.sender.down('#gen');
                    scope.sender.controller.assignAllComboData(scope.sender, 'gen', response.responseText);
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#TestThreeTree');
               ...
               var compRef = this.view.down('#TestThreeTree');
     ...
     resetForm: function(but) {
          ...
          form.down('#saveFormButton').setText('Save');
          form.reset();
          var grid = this.view.down('#TestThreeGrid');
          var tree = this.view.down('#TestThreeTree');
          tree.setSelection();
          grid.setSelection();
     },
     hideDataBrowser: function() {
          var form = this.view.down('#TestThree');
          var grid = this.view.down('#TestThreeGrid');
          var tree = this.view.down('#TestThreeTreeContainer');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#TestThreeForm');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, opts) {
                    ...
                    var compRef = currentView.down('#TestThreeTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          ...
          if (record.data.leaf) {
               var TestThree = this.view.down('#TestThreeForm');
               TestThree.reset();
               var gridPanel = this.view.down('#TestThreeGrid');
          ...
          var formPanel = this.getView().up().down('#TestThreeForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#TestThreeTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#TestThree').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#TestThreeGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
