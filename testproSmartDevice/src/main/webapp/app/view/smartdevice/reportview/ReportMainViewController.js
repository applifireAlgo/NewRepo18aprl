Ext.define('Testpro.view.smartdevice.reportview.ReportMainViewController',{
	...
	configureReportViewUI :function(reportmainview,isDrillDown,criteriaColumns){
		...
		Ext.Ajax.request({
			url:AppRestUrl+'secure/reportViewController/initializeReport',
			method:'POST',
			jsonData : {
	...
	configureReportUI : function(scope,reportUIJson,reportmainview){
		...
		if(reportUIJson.hasDataPoint){
			var datapointview = Ext.create('Testpro.view.smartdevice.reportview.datapointview.DataPointView',{
		...
		if(reportUIJson.hasGrid){
			...
			var dataviewpanel = Ext.create('Testpro.view.smartdevice.reportview.dataview.DataView',{
				title : 'Data',
				iconAlign:'top',
				reportUIJson:reportUIJson
		...
		if(reportUIJson.hasChart){
			var chartviewpanel = Ext.create('Testpro.view.smartdevice.reportview.chartview.ChartView',{
				title : 'Chart',
				iconAlign:'top',
				reportUIJson:reportUIJson
			//	icon:'resources/appicons/chart.png'
		...
		if(reportUIJson.searchCriteriaWidgets.length>0){
			var criteriaview = Ext.create('Testpro.view.smartdevice.reportview.querycriteria.QueryCriteria',{
		...
		if(reportUIJson.hasMap){
			var mapviewpanel = Ext.create('Testpro.view.smartdevice.reportview.mapview.MapView',{
				title : 'Map',
				reportUIJson:reportUIJson
			});
