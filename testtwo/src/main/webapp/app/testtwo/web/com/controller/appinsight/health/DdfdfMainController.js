Ext.define('Testtwo.testtwo.web.com.controller.appinsight.health.DdfdfMainController', {
     extend: 'Testtwo.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.DdfdfMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#DdfdfTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#DdfdfForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#DdfdfGrid');
               var tree = this.view.down('#DdfdfTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#DdfdfForm');
          var grid = this.view.down('#DdfdfGrid');
          var tree = this.view.down('#DdfdfTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#DdfdfGrid');
               ...
               var formPanel = this.getView().up().down('#DdfdfForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#DdfdfTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#DdfdfForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#DdfdfGrid').store;
          gridStore.removeAll();
