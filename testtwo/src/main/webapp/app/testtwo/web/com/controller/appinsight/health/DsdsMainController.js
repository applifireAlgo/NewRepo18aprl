Ext.define('Testtwo.testtwo.web.com.controller.appinsight.health.DsdsMainController', {
     extend: 'Testtwo.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.DsdsMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#DsdsTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#DsdsForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#DsdsGrid');
               var tree = this.view.down('#DsdsTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#DsdsForm');
          var grid = this.view.down('#DsdsGrid');
          var tree = this.view.down('#DsdsTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#DsdsGrid');
               ...
               var formPanel = this.getView().up().down('#DsdsForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#DsdsTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#DsdsForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#DsdsGrid').store;
          gridStore.removeAll();
