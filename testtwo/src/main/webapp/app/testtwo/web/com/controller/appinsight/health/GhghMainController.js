Ext.define('Testtwo.testtwo.web.com.controller.appinsight.health.GhghMainController', {
     extend: 'Testtwo.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.GhghMainController',
     ...
     rolesLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               success: function(response, scope) {
                    var compRef = scope.sender.down('#roles');
                    scope.sender.controller.assignAllComboData(scope.sender, 'roles', response.responseText);
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#GhghTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#GhghForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#GhghGrid');
               var tree = this.view.down('#GhghTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#GhghForm');
          var grid = this.view.down('#GhghGrid');
          var tree = this.view.down('#GhghTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#GhghGrid');
               ...
               var formPanel = this.getView().up().down('#GhghForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#GhghTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#GhghForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#GhghGrid').store;
          gridStore.removeAll();
