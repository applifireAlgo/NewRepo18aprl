Ext.define('Testtwo.testtwo.web.com.controller.appinsight.health.HghMainController', {
     extend: 'Testtwo.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.HghMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#HghTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#HghForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#HghGrid');
               var tree = this.view.down('#HghTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#HghForm');
          var grid = this.view.down('#HghGrid');
          var tree = this.view.down('#HghTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#HghGrid');
               ...
               var formPanel = this.getView().up().down('#HghForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#HghTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#HghForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#HghGrid').store;
          gridStore.removeAll();
