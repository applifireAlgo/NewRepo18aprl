Ext.define('Testtwo.testtwo.web.com.controller.appinsight.health.MynewMainController', {
     extend: 'Testtwo.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.MynewMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#MynewTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#MynewForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#MynewGrid');
               var tree = this.view.down('#MynewTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#MynewForm');
          var grid = this.view.down('#MynewGrid');
          var tree = this.view.down('#MynewTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#MynewGrid');
               ...
               var formPanel = this.getView().up().down('#MynewForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#MynewTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#MynewForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#MynewGrid').store;
          gridStore.removeAll();
