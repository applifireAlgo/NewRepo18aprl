Ext.define('Testtwo.testtwo.web.com.controller.appinsight.health.NewwMainController', {
     extend: 'Testtwo.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.NewwMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#NewwTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#NewwForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#NewwGrid');
               var tree = this.view.down('#NewwTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#NewwForm');
          var grid = this.view.down('#NewwGrid');
          var tree = this.view.down('#NewwTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#NewwGrid');
               ...
               var formPanel = this.getView().up().down('#NewwForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#NewwTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#NewwForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#NewwGrid').store;
          gridStore.removeAll();
