Ext.define('Testtwo.testtwo.web.com.controller.appinsight.health.NhhMainController', {
     extend: 'Testtwo.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.NhhMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#NhhTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#NhhForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#NhhGrid');
               var tree = this.view.down('#NhhTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#NhhForm');
          var grid = this.view.down('#NhhGrid');
          var tree = this.view.down('#NhhTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#NhhGrid');
               ...
               var formPanel = this.getView().up().down('#NhhForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#NhhTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#NhhForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#NhhGrid').store;
          gridStore.removeAll();
