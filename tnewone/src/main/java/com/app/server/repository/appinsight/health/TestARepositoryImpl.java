...
@Repository
@SourceCodeAuthorClass(createdBy = "shweta.zagade@algorhythm.co.in", updatedBy = "shweta.zagade@algorhythm.co.in", versionNumber = "7", comments = "Repository for TestA Transaction table", complexity = Complexity.MEDIUM)
public class TestARepositoryImpl extends SearchInterfaceImpl implements TestARepository<TestA> {
    ...
    @Transactional
    public List<TestA> findByTgen(String tgen) throws Exception, SpartanPersistenceException {
        try {
            javax.persistence.EntityManager emanager = emfResource.getResource();
            javax.persistence.Query query = emanager.createNamedQuery("TestA.findByTgen");
            query.setParameter("tgen", tgen);
            java.util.List<com.app.shared.appinsight.health.TestA> listOfTestA = query.getResultList();
            return listOfTestA;
        } catch (javax.persistence.PersistenceException e) {
    ...
    @Transactional
    public TestA findById(String tt) throws Exception, SpartanPersistenceException {
        try {
            javax.persistence.EntityManager emanager = emfResource.getResource();
