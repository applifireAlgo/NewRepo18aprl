...
public class TestAServiceImpl extends TestAService {
    ...
    @RequestMapping(value = "/findByTgen", method = RequestMethod.POST)
    @Override
    public HttpEntity<ResponseBean> findByTgen(@RequestBody FindByBean findByBean) throws SpartanPersistenceException, Exception {
        com.athena.framework.server.bean.ResponseBean responseBean = new ResponseBean();
        org.springframework.http.HttpStatus httpStatus = org.springframework.http.HttpStatus.OK;
        try {
            List<com.app.shared.appinsight.health.TestA> lsttesta = testArepo.findByTgen((java.lang.String) findByBean.getFindKey());
            responseBean.add("success", true);
            responseBean.add("message", "Successfully retrived ");
            responseBean.add("data", lsttesta);
        } catch (org.springframework.transaction.TransactionException e) {
