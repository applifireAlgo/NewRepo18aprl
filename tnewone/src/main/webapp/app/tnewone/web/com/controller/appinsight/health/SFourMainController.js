Ext.define('Tnewone.tnewone.web.com.controller.appinsight.health.SFourMainController', {
     extend: 'Tnewone.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.SFourMainController',
     init: function() {
          var form = this.view.down('#SFourForm');
     ...
     sffdfLoad: function() {
          ...
          Ext.Ajax.request({
               url: 'secure/Sone/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#sffdf');
                    scope.sender.controller.assignAllComboData(scope.sender, 'sffdf', response.responseText);
     ...
     sdsLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#sds');
                    scope.sender.controller.assignAllComboData(scope.sender, 'sds', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#sds');
     ...
     dfdLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#dfd');
                    scope.sender.controller.assignAllComboData(scope.sender, 'dfd', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#dfd');
     ...
     sdfsLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#sdfs');
                    scope.sender.controller.assignAllComboData(scope.sender, 'sdfs', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#sdfs');
     ...
     fsLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#fs');
                    scope.sender.controller.assignAllComboData(scope.sender, 'fs', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#fs');
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#SFourTree');
               ...
               var compRef = this.view.down('#SFourTree');
     ...
     resetForm: function(but) {
          ...
          form.down('#saveFormButton').setText('Save');
          form.reset();
          var grid = this.view.down('#SFourGrid');
          var tree = this.view.down('#SFourTree');
          tree.setSelection();
          grid.setSelection();
     },
     hideDataBrowser: function() {
          var form = this.view.down('#SFour');
          var grid = this.view.down('#SFourGrid');
          var tree = this.view.down('#SFourTreeContainer');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#SFourForm');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#SFourTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          ...
          if (record.data.leaf) {
               var SFour = this.view.down('#SFourForm');
               SFour.reset();
               var gridPanel = this.view.down('#SFourGrid');
          ...
          var formPanel = this.getView().up().down('#SFourForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#SFourTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#SFour').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#SFourGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
