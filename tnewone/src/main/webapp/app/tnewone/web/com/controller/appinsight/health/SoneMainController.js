Ext.define('Tnewone.tnewone.web.com.controller.appinsight.health.SoneMainController', {
     extend: 'Tnewone.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.SoneMainController',
     init: function() {
          var form = this.view.down('#SoneForm');
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#SoneTree');
               ...
               var compRef = this.view.down('#SoneTree');
     ...
     resetForm: function(but) {
          ...
          form.down('#saveFormButton').setText('Save');
          form.reset();
          var grid = this.view.down('#SoneGrid');
          var tree = this.view.down('#SoneTree');
          tree.setSelection();
          grid.setSelection();
     },
     hideDataBrowser: function() {
          var form = this.view.down('#Sone');
          var grid = this.view.down('#SoneGrid');
          var tree = this.view.down('#SoneTreeContainer');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#SoneForm');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#SoneTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          ...
          if (record.data.leaf) {
               var Sone = this.view.down('#SoneForm');
               Sone.reset();
               var gridPanel = this.view.down('#SoneGrid');
          ...
          var formPanel = this.getView().up().down('#SoneForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#SoneTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Sone').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#SoneGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
